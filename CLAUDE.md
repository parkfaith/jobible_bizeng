...
app/
  ...
  review/page.tsx             # 학습 캘린더(공부한 날=daily 연습 세션 기준 체크, 면접 제외) + 패턴 복습 목록 (서버 컴포넌트)
...
학습 캘린더 정리 (2026-07-01): (1) 하단 nav 라벨 "복습"→"학습"으로 통일(화면 제목 "학습 캘린더"와 일치, 4개 페이지). (2) ✓ 체크 기준에서 면접(interview) 제외 — daily 연습 세션만 습관 트래킹, 면접은 주 3회 제한·토큰 부담으로 아껴 쓰므로 성과는 통계 화면에서만 확인. 쿼리에 mode='daily' 필터 추가(review/page.tsx).
다음 작업 후보: iPhone 실기기 테스트 (터치 보정·마스터 모드·JD 면접·nav 밀착·학습 체크 기준), Codex 코드리뷰.
